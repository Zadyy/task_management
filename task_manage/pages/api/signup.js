...
router.post('/api/signup', (req, res) => {
  ...
  pool.query(sql, [email], (error, results) => {
    ...
    if (email == results.email) {
        res.status(401).json({ error: 'Email already registired' });
     } else {
      const enter_data = 'call sign_up(?,?)';
      pool.query(enter_data, [email, password], (error, results) => {
        if (error) {
            console.error('Error executing login:', error);
      return res.status(500).json({ error: 'Internal server error' });
        } else {
            res.json({ redirect: '/successful' });
        };
      })
    }
  });
